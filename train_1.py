...
def main(args):
    ...
    sampling_mode = MEDIUM
    MAX_ITER_RATE = 0.5
    inter = np.uint8(np.sqrt(np.linspace(0,MAX_ITER_RATE,NUM_ITER))*SAMPLING_TASKS)
    ...
    for batch in trange(config['num-batches']):
        ...
        if first_train or (num_prior == 0):
            ...
            sample,score = get_sample_buff(buffer,num_prior,sampling_mode)
